...
export default function LidCollector({ id, caption }: Props) {
  ...
  // NOTE: We prevent default so the input doesn't get focus / button doesn't submit.
    ...
    <section className="relative mt-10 w-full text-slate-900">
      <div className="mx-auto w-full max-w-6xl px-6">
        <div className="relative w-full overflow-visible rounded-3xl bg-gradient-to-r from-[#0b63ff] via-[#00a9d6] to-[#19d46a] px-6 py-10 shadow-[0_18px_50px_rgba(0,0,0,0.18)] md:px-12 md:py-12">
          ...
          {!success ? (
            <form id={id} onSubmit={() => {}} onKeyDown={preventEnterSubmit}>
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-4 lg:gap-[18px]">
                ...
                <button
                  ...
                  type="submit"
                  disabled={submitting}
                  // ✅ open calendar on click (and stop submit)
                  onMouseDown={hijackToCalendar}
                  onTouchStart={hijackToCalendar}
                >
                  {submitting ? (
      ...
      {calendarOpen && (
        ...
        >
          <div className="relative w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-[0_20px_70px_rgba(0,0,0,0.35)]">
            <button
              type="button"
              onClick={closeCalendar}
              className="absolute right-3 top-3 inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/90 shadow hover:bg-white"
              aria-label="Close"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <div className="h-[565px] w-full">
              <iframe
                ...
                frameBorder="0"
                allow="clipboard-write; payment; geolocation; microphone; camera"
              />
            </div>
          </div>
        </div>
